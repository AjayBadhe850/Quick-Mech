...
const Login = () => {
  ...
  const handleGetOTP = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await authAPI.sendOtp(contactValue.trim(), username.trim());
    ...
    try {
      ...
      if (data.success) {
        const userData = {
          ...
          email: data.user?.email || (contactValue.includes('@') ? contactValue : null),
          role: data.user?.role?.toLowerCase() || (contactValue === '7396230359' ? 'admin' : 'user'),
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (data.success) {
        setOtp(['', '', '', '', '', '']);
        setAttemptsRemaining(5);
        setSuccessMessage(`New OTP sent to ${contactValue}`);
