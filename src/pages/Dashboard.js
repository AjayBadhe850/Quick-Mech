...
const Dashboard = () => {
  ...
  const mechanicsList =
    JSON.parse(localStorage.getItem('adminMechanics') || 'null') || defaultMechanics;
  ...
  const getServiceIcon = (service) => {
    if (service.toLowerCase().includes("oil")) return "🛢️";
    if (service.toLowerCase().includes("brake")) return "🛞";
    if (service.toLowerCase().includes("engine")) return "⚙️";
    if (service.toLowerCase().includes("battery")) return "🔋";
    if (service.toLowerCase().includes("tire")) return "🚗";
    if (service.toLowerCase().includes("AC".toLowerCase())) return "❄️";
    return "🔧";
  };
  ...
  const getMechanicDistance = (mechanic) => {
    ...
    const match = mechanic.distance?.match(/([\d.]+)/);
